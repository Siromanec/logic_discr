...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.frame_logical_gates.grid(
            row=0, column=0, sticky="nswe",  padx=20, pady=20)

        # Label for the framw
        self.label_logical_frame = customtkinter.CTkLabel(
            master=self.frame_logical_gates, text="Logic Gates", text_font=("Roboto Medium", 13))
        ...
        self.input_controls.grid(
            row=1, column=0, sticky="nswe",  padx=20, pady=20)

        # Label for the framw
        self.label_input_controls = customtkinter.CTkLabel(
            master=self.input_controls, text="Input Controls", text_font=("Roboto Medium", 13))
        ...
        # Label for the framw
        self.label_output_controls = customtkinter.CTkLabel(
            master=self.output_controls, text="Output Controls", text_font=("Roboto Medium", 13))
        self.label_output_controls.grid(row=0, column=0)

        # Light Bulb button
        self.light_bulb_button = customtkinter.CTkButton(
            master=self.output_controls,
            text="Light Bulb",
            compound=local_compound,
            height=local_height,
            width=local_width,
            fg_color=local_fg_color, 
            command=self.curr_com_put_bulb
        )
        ...
        self.canvas = Canvas(master=self, height=700,
                             width=1000, bg="light grey")
        self.canvas.grid(row=0, column=2, rowspan=3)

    def put_bulb(self, event):
        """Puts image bulb on the canvas"""
        print("clicked at", event.x, event.y)
        App.coords.append(event.x)
        App.coords.append(event.y)
        if not getattr(self, 'one', None):  # First call
            self.one = ImageTk.PhotoImage(Image.open("light_bulb.png").resize((50, 100)))
        self.canvas.create_image(event.x, event.y, image= self.one)
        App.coords.clear()
